...
impl AsrModel for GigaAmModel {
    ...
    fn transcribe(
        ...
        if samples.len() <= chunk_samples {
            ...
            // Безопасное усечение строки для логов (чтобы не разорвать UTF-8 символ кириллицы)
            let log_text: String = text.chars().take(80).collect();
...
impl GigaAmModel {
    ...
    fn transcribe_chunk(&self, samples: &[f32]) -> AsrResult<String> {
        ...
        // На CUDA веса грузятся в BF16, на Metal в F32
        let mel_dtype = if self.device.is_cuda() { DType::BF16 } else { DType::F32 };
        let mel = mel.to_dtype(mel_dtype)?;
