...
impl TdtGreedyDecoder {
    ...
    pub fn decode(
        ...
        while time_idx < t_total {
            ...
            if step_count < 5 {
                let token_logits_f32 = token_logits.to_dtype(candle_core::DType::F32)?;
                let _token_max = token_logits_f32.max(D::Minus1)?.to_scalar::<f32>()?;
                let blank_score = token_logits_f32.i(self.blank_idx)?.to_scalar::<f32>()?;
                // Top-5 tokens
                let logits_vec: Vec<f32> = token_logits_f32.to_vec1()?;
