...
impl RelPositionMultiHeadAttention {
    fn load(d_model: usize, n_heads: usize, d_k: usize, vb: VarBuilder) -> Result<Self> {
        let linear_q = linear_with_bias(d_model, d_model, vb.pp("linear_q"))?;
        let linear_k = linear_with_bias(d_model, d_model, vb.pp("linear_k"))?;
        let linear_v = linear_with_bias(d_model, d_model, vb.pp("linear_v"))?;
