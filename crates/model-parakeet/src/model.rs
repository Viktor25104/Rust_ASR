...
impl ParakeetModel {
        ...
        let mel_extractor = {
            ...
            match (mel_fb, window) {
                (Ok(fb), Ok(w)) => {
                    ...
                    let fb_f32 = fb.to_dtype(DType::F32)?;
                    let w_f32 = w.to_dtype(DType::F32)?;
                    ParakeetMelExtractor::from_tensors(&config, &fb_f32, Some(&w_f32))?
                }
                (Ok(fb), Err(_)) => {
                    info!("Mel-фильтры из весов, окно сгенерировано");
                    let fb_f32 = fb.to_dtype(DType::F32)?;
                    ParakeetMelExtractor::from_tensors(&config, &fb_f32, None)?
                }
                _ => {
    ...
    fn transcribe_chunk(&self, samples: &[f32]) -> AsrResult<String> {
        ...
        info!(
            ...
            enc_f32.max(0)?.to_scalar::<f32>().unwrap_or(f32::NAN),
            enc_f32.mean_all()?.to_scalar::<f32>().unwrap_or(f32::NAN),
        );
