...
def infer_dimensions(state_dict: dict) -> dict:
    ...
    # Decoder embeddings
    emb_key = "decoder.prediction.embed.weight"
    emb0_key = "decoder.prediction.embeds.0.weight"
    emb1_key = "decoder.prediction.embeds.1.weight"
    
    if emb_key in state_dict:
        dims['vocab_size'] = state_dict[emb_key].shape[0]
        dims['pred_hidden'] = state_dict[emb_key].shape[1]
    else:
...
def create_config_json(config_info: dict, dims: dict, output_path: str):
    ...
    config = {
        ...
        "encoder": {
            ...
            "feat_in": dims.get('subsampling_feat_in') * 8 // dims.get('subsampling_conv_channels', 256) if dims.get('subsampling_feat_in') else 128,
        },
        "decoder": {
            "decoder_type": "lstm",
...
def main():
    ...
    if args.model and os.path.exists(nemo_path):
        os.remove(nemo_path)
        print(f"Удалён исходный файл: {nemo_path}")
